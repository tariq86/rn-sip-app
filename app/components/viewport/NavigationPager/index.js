...
class NavigationItem extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      opacityValue: new Animated.Value(props.selected ? 1.0 : 0)
    }
  }

  componentWillReceiveProps(nextProps) {
    if (this.props.selected !== nextProps.selected || this.props.offset !== nextProps.offset) {
      console.log('animation start!')
      Animated.parallel([
        // Animated.timing(this.state.offsetValue, {toValue: nextProps.offset}),
        Animated.timing(this.state.opacityValue, {toValue: nextProps.selected ? 1.0 : 0, duration: 5000})
  ...
  render() {
    ...
    return (
      <View style={{position: 'absolute', top: 7, right: this.props.offset, flexDirection: 'row', height: 36}}>
        <View style={{width: 36, height: 36, borderRadius: 18, backgroundColor: "#FFF"}}>
          <Text>{tab[0]}</Text>
        </View>
        <Text style={{paddingLeft: 8, opacity: this.props.selected ? 1 : 0}} onLayout={onTextLayout} pointerEvents="none" numberOfLines={1}>{getNameOfTab(tab)}</Text>
      </View>
    )
  }
}
...
class NavigationPager extends Component {
  ...
  onTextLayout(name, event) {
    ...
    for (let name of NAVIGATION_TABS) {
  ...
  render() {
    ...
    return (
      <View style={{height: 50, flex: 1, flexDirection: 'row', backgroundColor: '#72a472'}}>
        {
